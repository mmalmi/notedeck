...
fn test_create_new_invite() -> Result<()> {
    ...
    assert_eq!(hex::encode(invite.inviter_ephemeral_public_key.bytes()).len(), 64);
    assert_eq!(hex::encode(invite.shared_secret).len(), 64);
    assert_eq!(invite.inviter.bytes(), alice_pk.bytes());
...
fn test_url_generation_and_parsing() -> Result<()> {
    ...
    assert_eq!(parsed_invite.inviter.bytes(), invite.inviter.bytes());
...
fn test_invite_accept_creates_session() -> Result<()> {
    ...
    assert_ne!(event.pubkey.to_bytes(), *bob_pk.bytes());

    let has_p_tag = event.tags.iter().any(|t| {
        let v = t.clone().to_vec();
        v.get(0).map(|s| s.as_str()) == Some("p")
            && v.get(1).map(|s| s.as_str()) == Some(&hex::encode(invite.inviter_ephemeral_public_key.bytes()))
    });
    assert!(has_p_tag);
...
fn test_invite_serialization() -> Result<()> {
    ...
    assert_eq!(deserialized.inviter_ephemeral_public_key.bytes(), invite.inviter_ephemeral_public_key.bytes());
    assert_eq!(deserialized.shared_secret, invite.shared_secret);
    assert_eq!(deserialized.inviter.bytes(), invite.inviter.bytes());
